...
- 待做:`annual_review`(接 `lesson_loop` → B4)。
- 待做(非阻断,Codex gate 记录):把候选 `source_refs` 从 **report 级聚合**收窄为
  **detector 级最小 provenance**(当前够重建但偏宽)。
...
- DONE (h) `tax_window` 探测器(**deadline / 预缴 / 文档复核,不是税务建议**)— 角色路由:
  ...
  - **第二轮 gate PASS(Codex 独立验收)**:targeted 47 tests OK、source_refs 探针(tax 候选带
    ...
    一致,适合后续统一收窄)。Release Manager 结构化提交。
