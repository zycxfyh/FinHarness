...
## RE2 设计稿(Architect,2026-06-21;设计 gate 用,**不开码、不改 API**)

RE2 = `concentration_high` 的**历史市场证据 provider adapter**——实现 RE1 的
`ResearchEvidenceProvider.provide(request) -> ResearchEvidenceResult`,对集中持仓的
top symbol 回答 `historical_risk_profile`。风险高于 RE1(接真实引擎 + 网络 + 数据质量),
所以先用 RE1 给的新流程写 **surface inventory + traceability**,再开码。

### 核心设计决定
- **只读描述性,硬排除优化器/预测**:只触达 `market_data`(OHLCV/volume + `DataReceipt`)、
  `indicator_layer.compute_ohlcv_risk_return_features`(drawdown / max_drawdown / rolling
  CVaR)、`compute_library_core_indicators`(ATR)、`portfolio_risk.returns_from_close_prices`。
  **绝不调用** `portfolio_risk.optimize_riskfolio_allocation`(给目标权重=advice)。
- **注入式数据源(可测、网络隔离)**:provider 接受一个 `MarketHistorySource` 依赖;默认实现走
  `market_data`(可能联网/缓存),**单测注入 fixture OHLCV**,不打网络。这条直接服务"质量不依赖
  当天模型/网络状态"的规则。
- **无数据→data_gap,绝不崩、绝不编**:取不到历史、历史太短算不了 CVaR、symbol 找不到 →
  各自一条 `data_gaps`,result 仍返回(空 items + gaps)。RE1 的 redline 是 evidence 内容的兜底。
- **grade 固定 `historical_market_data`**,每条 evidence 带 RE1 要求的 non_claims;`value` 只含
  `realized_volatility/max_drawdown/conditional_var/average_volume`(均在 RE1 closed `kind` 枚举内)。
- **provider status 进 lineage/receipt**(新规则):source、包版本、as_of、数据质量/reconciliation 标记。

### Codex 设计 gate 要求锁定的 4 项(gate 第一轮 finding)
1. **输入面守住(在调外部 provider 之前)**:
   - `subject` 必须是 normalized ticker/fund symbol——复用 `restricted_symbols.normalize_symbol`
     + 限定字符集(A–Z0–9 与少量 `.`/`-`)、长度上限、统一大写;**非法 symbol → `data_gap`,
     绝不调用外部 provider**。
   - `time_window` 收成**闭集** `ResearchTimeWindow = Literal["trailing_1y","trailing_3y"]`
     ——这是对 RE1 `time_window: str` 的**小修正(str→Literal),随 RE2 一起 ship**,与 RE1 已有的
     `question`/`kind`/`grade` 闭集一致;拒绝自由文本/未来/预测式窗口。
2. **输出形状定死**:**一个 aggregate item**,`kind="historical_risk_profile"`,`value` 白名单四键
   `{realized_volatility, max_drawdown, conditional_var, average_volume}`(不是四个 metric item)。
   测试/前端/RE3 都按这一种形状。
3. **claim 用过去时模板,禁未来感**:模板形如
   `Over {window}, {symbol}'s observed max drawdown was {x}; realized volatility was {y}; ...`。
   **禁** "risk is high / likely / expected" 这类未来式表述。RE1 redline 只兜词,不能替代模板——
   claim template 作为 RE2 的额外约束 + 测试。
4. **receipt/lineage 边界说清**:RE2 **本身不写 `Proposal`、不新增端点**;**默认** market-data source
   *可以*写它自己的 **market-data receipt**(沿用 `market_data` 既有行为);provider 输出**只通过
   `source_refs + lineage` 引用**这些 receipt,不自己造决策类 receipt。

### Surface Inventory(所有进/出/外部面)
- **输入**:`ResearchEvidenceRequest{detector_kind, subject=symbol(来自候选,可信), question, time_window}`。
- **外部调用/数据源**:历史 OHLCV 拉取(网络:OpenBB/yfinance via `market_data`)/ 缓存 receipt。
  失败面:网络不可达、symbol 不存在、历史不足、数据陈旧、reconciliation 失败。
- **引擎只读面(描述性)**:见核心决定;**排除面**:优化器、任何 backtest-as-forecast、预测。
- **输出**:`ResearchEvidenceResult{items: ResearchEvidence[], data_gaps}` —— 已被 RE1 redline 守。
- **非输出/豁免**:不写任何 Proposal、不改 `/proposals`、不新增端点(RE2 只是 provider;接线是 RE3)。

### Traceability(设计承诺 → 计划代码点 → 测试 → gate 探针)
| 设计承诺 | 计划代码 | 测试 | gate 探针 |
| --- | --- | --- | --- |
| 不调用优化器/预测 | provider 不 import `optimize_riskfolio_allocation` | 模块不引用优化器 | AST/grep 探针:无优化器调用 |
| 描述性 value 键白名单 | 只产 vol/dd/cvar/volume | value 键 ⊆ 白名单 | 喂数据→断言键集 |
| 无数据/短历史/坏 symbol → data_gap | fetch/compute try→gap | bogus symbol、短历史 → 有 gap、无异常 | 注入空/短 fixture |
| 不打网络可测 | 注入 `MarketHistorySource` | 单测用 fixture OHLCV | 断言无网络调用 |
| grade + 必带 non_claims | provider 填 grade + REQUIRED_NON_CLAIMS | 产出项过 RE1 校验 | 构造并 validate |
| provider status 可复盘 | lineage 填 source/version/as_of/quality | lineage 非空且含 status | — |
| 非法 symbol 不出网 | `normalize_symbol` + 校验,失败→data_gap | bad symbol → gap 且无外部调用 | mock source 断言未被调用 |
| time_window 闭集 | `ResearchTimeWindow` Literal(amends RE1) | 非法窗口被拒/降级 | 构造非法窗口 |
| 单 aggregate item 形状 | 产 1 个 `historical_risk_profile` item | 恰 1 item,value 键=白名单四件 | 断言 len(items)==1 + 键集 |
| claim 过去时模板,禁未来感 | 模板化 claim,禁 is high/likely/expected | claim 匹配模板、未来词被拒 | 探针喂未来式→拒 |

### 任务拆分(设计 gate 过后)
- RE2a `MarketHistorySource` 抽象 + fixture 实现(纯,可测)。
- RE2b `HistoricalRiskProfileProvider.provide`(只读描述性,失败→data_gap),回归测试含上表每行 + 优化器排除红线探针。
- (RE3 仍是后续:`enrich_with_research_evidence` 被动接线 + 前端只读展示。)

## RE3 设计稿(Architect,2026-06-22;设计 gate 用,**不开码、不改 API**)

RE3 = 把 RE2 的 provider **被动接进** allocation candidate → Proposal → cockpit 链路,并在前端
**只读展示**。风险点从"provider 本体"上移到"证据如何进入 Proposal / cockpit 且**不被误读为建议**、
**不反向驱动** candidate 管线"。RE2(commit `7e7595e`)与 RE1(`80b1793`)已 ship,RE3 不改它们的契约。

> 本段按 [mini-RFC 模板](../templates/mini-rfc.md) 组织(C2/C3 要求)。下列小节对应模板 8 节。

### 1. Change Class
**C3** —— 命中 [change-control](../engineering/change-control.md) 触发器:改变默认行为(scan 加 enrichment 接缝)、
引入联网面(opt-in)、改证据语义与**用户可见解释**(投资风险描述,可能被误读为建议)。故走 mini-RFC +
threat/surface inventory + independent gate + CI hardening。

### 2. Current behavior
`AllocationCandidate.research_evidence` 恒为 `()`;[allocation.py:545](../../src/finharness/allocation.py#L545)
把它 `list(...)` 塞进 proposal evidence(因恒空所以现在不炸)。前端 `STRUCTURAL_EVIDENCE_KEYS` 已含
`research_evidence` 但**无渲染块**。`decisions:scan` 纯离线确定。

### 3. Target behavior
默认路径(无 `--with-research`):`NoopResearchEnricher` → candidate 不变、proposal evidence **保留今天的
shape**(`research_evidence: []` 照写,因为今天就写;**不**新增 `research_evidence_gaps` 键、**不**追加 research
source_refs)→ 输出与今天逐字节一致。opt-in 路径(`--with-research`):`ProviderResearchEnricher` 经 capability
routing 只对 `concentration_high` 调 RE2 provider,把结果包成 typed attachment 进 proposal,前端只读展示。

### 5. Default Path Invariant
**默认行为完全不变**,可证:`NoopResearchEnricher.enrich` 返回空 attachment;接线点**保留今天的 shape**——
`research_evidence` 键**照写为 `[]`**(今天 `list(())` 就是 `[]`),`research_evidence_gaps` 键**不存在时不新增**
(今天本就没有这个键),source_refs 不追加。故默认 scan 的 proposal evidence 与 RE3 前**逐字段相等**(快照测试
锁,见 Traceability 首行)。
> 注意陷阱:把空 attachment 序列化成"省略 `research_evidence` 键"会**改变** evidence shape/content hash →
> 触发新 revision,就**不是**逐字节一致了。所以"键恒在、值为 `[]`"是默认不变的前提,不可省。

### 北极星不变量(RE3 必须守)
- **依赖方向单向**:candidate *拉取* 证据;research 引擎**不驱动** candidate、不改 candidate 的
  `claim`/`assumptions`/`limitations`、不进 cockpit 的任何动作面。
- **证据是加性上下文,不是门**:research evidence 缺失/失败**绝不**改变 candidate 是否生成或记录。
- **默认路径离线、确定、可水合**:`decisions:scan` 默认 **enrichment 未启用**(`NoopResearchEnricher`,
  非 Null),输出与今天逐字节一致;真实联网 provider 仅显式 opt-in。

### 设计取向(Process Designer 校正,2026-06-22)

原则:**做最小的不可逆风险,不是最小 diff**。RE3 跨后端/provider/Proposal/前端,若塞进 `allocation.py`
三个 if,短期快、长期每接一个新证据 provider 都要重谈"默认路径/gap/source_refs/前端披露"这些老问题。
因此 RE3 落一个**小而完整的稳定接缝**:**Research Enrichment Subsystem**,让 RE4/RE5 复用而非重议。
`allocation.py` 不直接知道 provider 细节,只消费一个 typed attachment。

### 子系统:`research_enrichment.py`(新模块,RE3 的稳定接缝)

- **`ResearchEnricher`(Protocol)** — `enrich(candidate) -> ResearchEvidenceAttachment`。两实现:
  - **`NoopResearchEnricher`(默认)** — 不调用任何 provider、不改 candidate、**不产 gap**,返回空 attachment。
    语义 = "**enrichment 未启用**"。默认 scan 输出与今天逐字节一致。
  - **`ProviderResearchEnricher`(显式 opt-in)** — 持有一个 `ResearchEvidenceProvider`,经 capability
    routing 决定是否调用,把结果包成 attachment。
- **`ResearchEvidenceAttachment`(typed,frozen)** — `items: tuple[ResearchEvidence, ...]`、
  `data_gaps: tuple[str, ...]`、`source_refs: tuple[str, ...]`。**它**负责:`to_evidence_payload()`
  (`[it.model_dump() for it in items]`,空 items → `[]`)、source_refs 去重、gap 走 RE1 narrow 红线。
  `allocation.py` 只消费 attachment,不手拼细节。**接线契约(保默认不变)**:`research_evidence` 键
  **恒写**(空 → `[]`,= 今天 `list(())`);`research_evidence_gaps` 键**仅 data_gaps 非空时才写**(空时不新增,
  = 今天没有此键);source_refs **仅有 research refs 时才追加**。空 attachment(Noop)因此与今天逐字节一致。
- **`build_research_request(candidate) -> ResearchEvidenceRequest | None`** — capability routing 的请求构造:
  仅 `concentration_high` 且 `evidence["top_symbol"]` 存在时返回请求(`subject=top_symbol`,
  `question="historical_risk_profile"`,`time_window` 默认 `trailing_3y`);其余 detector 返回 `None`。
- **`sanitize_gap(exc) -> str`** — provider 异常**只生成安全模板**(如 `"research enrichment unavailable
  for this candidate"`),**绝不**搬异常原文进 Proposal/前端(防内部细节/路径泄漏,且过 RE1 narrow 红线)。

### Capability routing(路由层判定,不靠 provider 自报 unsupported)

`ProviderResearchEnricher.enrich` 先调 `build_research_request`:
- `cash_buffer_low` / `cash_overweight` / `rate_exposure_high` / `tax_window` / `insurance_gap` → `None`
  → **根本不调用 provider、不加任何 gap**。无关候选不被 research 噪声污染。
- `concentration_high` 且有 `top_symbol` → 调用 provider,包成 attachment。
- RE2 provider 自身的 detector scope-guard 保留为**纵深防御**(belt-and-suspenders),不是主判定;
  正常路径下根本不会走到它的 off-scope 分支。

### Noop vs Null 语义分离(关键)

| 层 | 名字 | 语义 | 默认路径行为 |
| --- | --- | --- | --- |
| enricher | `NoopResearchEnricher` | enrichment **未启用** | 不改 candidate、`research_evidence:[]` 照旧、无 gaps 键 |
| provider | `NullResearchEvidenceProvider`(RE1) | 启用了但**无 provider** | routing 命中 concentration → 披露 gap |

默认 `decisions:scan` 用 **Noop**(输出零变化)。`--with-research` 用 `ProviderResearchEnricher`,其内部
provider 可是真 `HistoricalRiskProfileProvider`(联网),或 `NullResearchEvidenceProvider`("启用但没配
provider"→ 对 concentration 披露 gap)。两种语义不再混为一谈。

### PM 5 项 ⇄ 子系统映射

1. 离线/opt-in → **默认 Noop 子系统**;`record_decisions.py --with-research` 才注入 `ProviderResearchEnricher`。
2. 失败仍记录 → enricher 吞异常经 `sanitize_gap`→ attachment.data_gaps;candidate 记录与 attachment **解耦**。
3. 进 Proposal → `record_allocation_candidates` 消费 `attachment.to_evidence_payload()`,写既有
   `evidence["research_evidence"]`(恒在,空=`[]`);`research_evidence_gaps` 仅非空时写;**无新字段/端点**;
   不动 candidate 核心字段。
4. 前端披露 → 专属只读块,grade/non_claims **强制常显贴 claim**、value 仅白名单四键、**零动作面**。
5. jsdom → 正例文本断言 + 强制披露同块断言 + 负例(无 button/动作链接、越界 value 键不显示)+ gap 例。

### 4. Surface Inventory(进/出/外部面)
- **输入**:`AllocationCandidate` + 注入的 `ResearchEnricher`(默认 Noop)。
- **新代码点**:`research_enrichment.py`(子系统)、`record_allocation_candidates(..., enricher: ResearchEnricher | None = None)`(函数内默认 `NoopResearchEnricher()`)
  消费 attachment、`record_decisions.py` 的 `--with-research`、`app.js` 渲染块。`allocation.py` **不 import** RE2 provider。
- **外部面**:仅 `--with-research` + 真 provider 时经 RE2 `MarketDataHistorySource` 联网(面属 RE2/market_data,
  RE3 不新增)。默认 Noop → 无外部面。
- **排除面**:不新增端点、不改 Proposal schema、不改 candidate 的 claim/假设/限制、不写新 receipt 类型、
  cockpit 无动作面。

### 6. Traceability Matrix(设计承诺 → 计划代码点 → 测试 → gate 探针)
| 设计承诺 | 计划代码 | 测试 | gate 探针 |
| --- | --- | --- | --- |
| 默认 = 未启用,输出零变化 | `NoopResearchEnricher` 默认 | 默认 scan proposal evidence 与 RE3 前**逐字段相等**(`research_evidence:[]` 保留、无 gaps 键) | 快照对比 enrich 前后 payload |
| 无关候选不调 provider | `build_research_request` 非 concentration→None | 非 concentration → provider 调用计数=0 | spy provider 断言未被调 |
| 只用 top_symbol 发请求 | routing 取 `evidence["top_symbol"]` | concentration → request.subject==top_symbol | 断言请求字段 |
| 失败仍记录 + 脱敏 | enrich try→`sanitize_gap`→attachment | provider 抛 → candidate 仍写、gap 为安全模板**不含异常原文** | 注入抛异常 provider,断言 gap 文本 |
| 证据不反向驱动 candidate | enrich 不改 candidate 核心字段 | enrich 前后 claim/假设/限制相等 | diff candidate 核心字段 |
| 无 Pydantic 对象泄漏 | `attachment.to_evidence_payload()` model_dump | proposal evidence `json.dumps` 往返成功 | JSON 往返探针 |
| source_refs 追加且去重 | attachment 负责去重 | 多 item 重复 ref → proposal source_refs 去重 | 断言去重后集合 |
| 空证据不被读成无风险 | gaps 入 `research_evidence_gaps` | 空 items+gap → evidence 含可见 gap | 断言 gap 非空 |
| 披露强制常显 / 无动作面 | 前端块 non_claims/grade 必渲染、无 button | jsdom 正例+强制披露+负例 | DOM 文本/查询断言 |
| value 不越白名单 | 前端只渲染四键 | jsdom 越界键→不显示 | 负例 DOM 断言 |
| 无新端点/schema | 复用 `/proposals` + 既有键 | 现有 proposal API 测试不回归 | grep 无新 route/字段 |
| **attachment 自身守红线**(防 rogue enricher) | `__post_init__` 复用 `ResearchEvidenceResult` 校验 data_gaps+items;source_refs 必派生自 items | `data_gaps=("buy SPY now",)` 构造即拒;free-form source_refs 拒 | 构造 advice gap / 游离 ref → 必失败 |

### 7. Test / Gate Plan
- **进 `task check`**:RE3a/RE3b 后端单测(Traceability 各行,尤其默认路径快照逐字段相等)+ RE3c jsdom
  (正例/披露/负例/gap)。
- **进 `task governance:check`(EOS G4,首批,RE3 实现时顺手做)**:默认 scan 不联网 + 不新增 research gap;
  evidence JSON 序列化往返(无 Pydantic 泄漏);前端 research block 无 button/动作链接;
  `research_enrichment.py` AST 探针不 import optimizer/Proposal/route。
- **Design gate**:默认行为不变、误读风险、依赖方向、回滚(本轮已过修订)。
- **Implementation gate**(独立):设计符合度、测试覆盖承诺、无夹带、复现 REAL_EXIT。
- **实现说明(采纳 gate 非阻断建议)**:接线签名用 `enricher: ResearchEnricher | None = None`,函数内
  实例化 `NoopResearchEnricher()`,避免可变默认参数对象的未来状态风险。

### 8. Not claimed / Debt
- **不**主张:research evidence 是建议/预测;不主张联网默认开启;不主张前端可由此触发任何动作。
- 已知债务(明确接受):`MarketDataHistorySource` 仍 best-effort、不单测网络(RE2 已记);
  capability routing 当前只认 `concentration_high`,接 RE4/RE5 新证据源时**复用**子系统、按需扩 routing。
- 后续 slice:第一批 `task governance:check` 探针在 RE3 实现时顺手做(EOS G4)。

### 任务拆分(设计 gate 过后)
- RE3a **子系统** `research_enrichment.py`:`ResearchEnricher` Protocol + `NoopResearchEnricher` +
  `ProviderResearchEnricher` + `ResearchEvidenceAttachment` + `build_research_request` + `sanitize_gap`;
  纯/可测,单测锁上表后端各行(架构边界,非"补当前 bug")。
- RE3b **接线** `record_allocation_candidates(..., enricher: ResearchEnricher | None = None)`(函数内默认 `NoopResearchEnricher()`) 消费 attachment +
  既有序列化点改走 `to_evidence_payload()`;`record_decisions.py` 加 `--with-research`(注入
  `ProviderResearchEnricher(HistoricalRiskProfileProvider(MarketDataHistorySource()))`)。
- RE3c **前端** `app.js` research-evidence 只读渲染块 + jsdom 测试(正例/披露/负例/gap),纳入 `task check`。
...
- DONE(作者侧)(l) **RE2 历史市场证据 provider**(`concentration_high` 的 `historical_risk_profile`)。
  - **RE1 小修**:`research_evidence.py` 把 `time_window: str` 收成闭集
    `ResearchTimeWindow = Literal["trailing_1y","trailing_3y"]`(request 与 evidence 两处),
    field policy 记 `closed_literal`,`_prose_has_no_advice` 只剩 `claim`(time_window 闭集后无需全文红线)。
    随 RE2 一起 ship,与 `question`/`kind`/`grade` 闭集一致。RE1 既有 22 项测试不回归。
  - **RE2a 注入式数据源**:`research_history_provider.py` 定义 `MarketHistorySource`(Protocol)+
    `MarketHistory`(OHLCV + provider status:source/as_of/包版本/reconciliation/source_refs)。
    `FixtureMarketHistorySource`(纯、网络隔离,单测注入)+ `MarketDataHistorySource`(默认 best-effort
    薄适配 `market_data._fetch_openbb_history`+`normalize_ohlcv`,**不单测网络**)。
  - **RE2b provider**:`HistoricalRiskProfileProvider.provide`——`normalize_research_symbol`
    (`normalize_symbol` + ticker 正则/长度,**非法→data_gap 且 source 未被调用**)、time_window 闭集→
    lookback、source 失败/空/短历史→各自一条 `data_gap` 不崩、复用 `returns_from_close_prices` 算
    描述性 vol/maxDD/CVaR/avg_volume(**恰一个 aggregate item**,value 白名单四键)、过去时 claim 模板
    + `_FUTURE_CLAIM_MARKERS` 防未来式、grade=`historical_market_data`+必带 non_claims、lineage 填
    provider status。**不 import 优化器、不写 Proposal、不新增端点**。
  - 回归测试 `tests/test_research_history_provider.py` 16 项,覆盖设计稿 Traceability 每行:
    单 aggregate item + value 键集、lineage provider status、过去时 claim、非法 symbol 短路且 spy
    source 未被调用、未知/短/空历史→data_gap、time_window 闭集拒非法、claim 模板拒未来词、
    **AST 标识符探针**(非裸 grep:排除 docstring;断言 `optimize_riskfolio_allocation`/`Proposal`/
    `APIRouter`/`FastAPI` 不在代码标识符内)、产出项过 RE1 校验 round-trip。
  - **Codex 实现 gate 第一轮 BLOCKED → 作者侧修 3 blocker**:
    (1) 缺 `detector_kind == "concentration_high"` 校验——off-scope detector(如 `cash_buffer_low`)
    仍产出证据;加 scope guard:非支持 detector→`data_gap` 且 source 未被调用。
    (2) OHLCV 缺 `volume` 列直接 `KeyError` 崩——改为校验 `REQUIRED_OHLCV_COLUMNS=(close,volume)`,缺列→`data_gap`。
    (3) 非数值 `volume`→`average_volume=nan`、claim 出现 "was nan"(编出不可用描述值)——加 `math.isfinite`
    全量校验,任一统计非有限→`data_gap`,不渲染 claim。新增 3 项回归测试(off-scope detector spy 未调用、
    缺列不崩、非数值 volume→non-finite gap)。
  - **gate 第二轮卫生项**:RE1 旧测试 `test_research_evidence.py` 有 3 处 pre-existing unused
    `type: ignore`(107/113/141;RE1 只对 `src` 跑 mypy 从未暴露)——经复现 Codex 的确切四文件
    `uv run mypy <re2 src> <re1 src> <re2 test> <re1 test>` 命令删除;**只删这 3 处**(单跑 test 文件会
    误报 122/200,因缺 model 上下文,以四文件命令为准)。
  - 验证:`ruff` 四文件 EXIT=0;`mypy` 四文件 EXIT=0 + 默认 `task typecheck`(142 文件)EXIT=0;
    `unittest tests.test_research_history_provider
    tests.test_research_evidence` REAL_EXIT=0(39 项);全量 `discover -s tests` REAL_EXIT=0(536 项,
    RE1 闭集改动无回归;本机 `uv run` 需 `PYTHONPATH=src` 绕开 VIRTUAL_ENV 串环境,非代码问题)。
    见记忆 [[finharness-verify-before-claiming-fixed]]、[[verify-real-exit-code-not-piped-tail]]。**待 Codex 实现 gate 复验**。
- DONE(作者侧)(m) **RE3 Research Enrichment Subsystem(C3,首例走 EOS v0 mini-RFC)+ EOS G1–G4**。
  - **流程**:EOS v0 文档 G1/G2/G3 已提交 `14fe246`;RE3 作为第一例按 mini-RFC 走 design gate(默认路径
    shape blocker 一轮修正后 **PASS**)。
  - **RE3a 子系统** `research_enrichment.py`:`EnrichableCandidate`(窄结构 Protocol,子系统**不 import**
    allocation)、`ResearchEvidenceAttachment`(`__post_init__` 复用 RE1 契约守 data_gaps+items 红线、
    source_refs 必派生自 items)、`NoopResearchEnricher`(默认)、`ProviderResearchEnricher`(opt-in)、
    `build_research_request`(capability routing,仅 `concentration_high`+`top_symbol`)、`sanitize_gap`(异常脱敏)。
  - **RE3b 接线** `allocation.py`:`record_allocation_candidates(..., enricher=None)` 默认 Noop;**保 shape**
    (`research_evidence` 恒写空=`[]`、`research_evidence_gaps` 仅非空写、source_refs 去重追加)。CLI
    `record_decisions.py --with-research`(默认离线,opt-in 懒加载真实 provider)。
  - **RE3c 前端** `app.js` `renderResearchEvidence` 只读块(grade/non_claims 强制常显贴 claim、value 仅
    白名单四键、零 button/链接)+ jsdom `research_evidence.test.cjs`(正例/披露同块/负例无动作面+越界键/gap/默认无块)。
  - **EOS G4** `tests/test_governance_invariants.py` + `task governance:check`(纳入 `task check`):import 边界
    AST 探针、redline policy 覆盖、attachment 红线、无 Pydantic 泄漏、前端无动作面。
  - **Codex 实现 gate**:第一轮 BLOCKED(attachment 可绕过 data_gaps 红线)→ 加 `__post_init__` 校验 + 3 回归 →
    第二轮后端 **PASS**。
  - 验证:`task typecheck`(src 143)EXIT=0;targeted REAL_EXIT=0;全量 `discover -s tests` REAL_EXIT=0(557 项);
    `task test:frontend`/`governance:check` 通过;CLI 冒烟默认离线 exit 0。见记忆
    [[verify-real-exit-code-not-piped-tail]]、[[governed-code-robust-to-model-fluctuation]]。**待 RE3c/G4 实现 gate + 整体 release gate**。
