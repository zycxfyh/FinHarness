...
- **lesson draft receipt 逐条 drill-down**:R3a **不做**(闭环状态已由 annual_review receipt 的
  `lessons_open/closed` 提供);逐条 lesson 展开(读 `data/receipts/lessons`)留作后续增量。
- 小 follow:R2c 余项(queue 默认 `archive=active` 隐藏 archived)可随本 slice 顺手或单列。

### 任务拆分(设计 gate 过后)
- R3a 后端:`/review/*` 只读端点(主读最新 `annual_review` receipt + **provenance receipt_ref**;rule-change
  state ledger 作 drill-down(status/traceable);空/坏→empty/gap;不调用写/计算入口);单测含上表后端各行 +
  rule-change drill-down + OpenAPI 契约。**lesson draft receipt 的 drill-down 不在 R3a**(见 Debt)——闭环状态已
  由 annual_review receipt 提供,lesson 逐条展开是后续增量。
