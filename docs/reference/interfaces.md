...
| CapitalMapInterface | Local deterministic views + canonical typed position-valuation admission | Optional admitted net worth, per-currency totals, valuation blockers, cash runway, concentration, liabilities, obligations, and data gaps; blocked valuation never becomes a unified headline, denominator, or allocation candidate | `position_valuation.py`, `exposure.py`, `task brief:daily` |
...
| ApiMutationIdentityInterface | FastAPI + authenticated identity provider | Optional `Idempotency-Key` admitted through the single exact APIRoute capability registry before body/receipt/handler; v2 claim bound to actor, canonical target/query, semantic headers, body hash, canonical route template, recovery mode, bounds, and resolver identity; durable locked CAS terminal transitions; exact replay; typed domain-truth reconciliation; historical v1 compatibility without capability backfill | `config/keyed-mutation-route-capabilities.json`, `api/keyed_mutation_capabilities.py`, `identity.py`, `task mutations:capabilities-check`, `task identity:reconcile`, [#387 ADR](../adr/2026-07-17-keyed-mutation-route-capabilities.md), [Durable write mini-RFC](../proposals/2026-07-13-durable-write-and-api-mutation-semantics.md) |
...
| CockpitInterface | FastAPI + static frontend | Exposure, IPS, proposals, review, legacy paper compatibility, and Execution Kernel views/routes; governed review writes resolve a no-store server principal/session binding, persist one bound logical mutation attempt before fetch, reuse its stable key only within that exact authentication epoch, revalidate the binding at the keyed middleware, retain ambiguous response-loss attempts for operator-driven typed reconciliation, and clear only the acknowledged attempt after response echo plus same-binding validation; execution mutations still require deployment capability | `task api:serve`, `task test:browser`, `frontend/api.js`, `frontend/actions.js`, `GET /identity/browser-mutation-binding`, [#388 ADR](../adr/2026-07-17-browser-mutation-identity-binding.md) |
