...
| Sec-debt R1:pydantic-settings symlink CVE | **C1**(单一 runtime 依赖安全升级,needs-compat-test) | in progress;register 第一条 remediation:`pydantic-settings 2.14.1 → 2.14.2`(CVE-2026,fixed 2.14.2;唯一 runtime 项);约束 `>=2.14.1`→`>=2.14.2` + uv.lock;`task check` 绿 |
