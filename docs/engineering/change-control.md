...
| Graph Registry R1(Task 4) | **C1**(架构治理元数据登记;无产品行为/依赖/runtime 变化) | DONE(merged PR #35,5fe6d95;`tests/_graph_registry.py` 把 audit 升级成可校验 registry,20 资产,`task governance:graphs`;判断资产≠删除授权;纠正 audit 失实 docs/archive 路径) |
| Security Debt Register(Task 5) | **C0**(security triage 文档;无代码/依赖变化) | DONE(merged PR #36,0443ae5;`docs/security/security-debt-register.md` 基于真实 dependabot API,15 open:4H/8M/3L,14 dev-scope/1 runtime;每个 high 有 owner/risk/next-action;triage-only;独立 track) |
| Sec-debt R1:pydantic-settings symlink CVE | **C1**(单一 runtime 依赖安全升级,needs-compat-test) | DONE(merged PR #37,`c3503d1`;register 第一条 remediation:`pydantic-settings 2.14.1 → 2.14.2`,唯一 runtime 项;约束 `>=2.14.1`→`>=2.14.2` + uv.lock;`task check` 绿) |
| Sec-debt R2:pnpm transitive dev CVEs | **C1**(dev-scope 传递依赖安全升级,pnpm.overrides) | DONE(merged PR #38,`28ae669`;register 第二条 remediation:`undici 7.28.0`/`hono 4.12.25`/`form-data 4.0.6`/`js-yaml 4.2.0`/`esbuild 0.28.1` 经 `pnpm.overrides` 钉死,清 14 个 dev 告警;promptfoo 传递依赖,不进产品 runtime) |
| Sec-debt C0 台账收口 | **C0**(security register + change-control 活页对账,无代码/依赖变化) | DONE(本 slice;#37/#38 合并后 live Dependabot open `15 → 0`;register 升 v1 CLOSED OUT;补 R1 DONE + R2 行) |
