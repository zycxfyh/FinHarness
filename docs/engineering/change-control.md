...
| Architecture Boundary Probes(Task 3) | **C1**(governance 探针;无产品行为变化) | in progress;System Map 边界进入 policy registry:`GOV-ARCH-003` API 不导入 headless engine,`GOV-ARCH-004` State Core 不反向依赖上层,`GOV-RESEARCH-001` 扩到 research contract/enrichment/provider |
