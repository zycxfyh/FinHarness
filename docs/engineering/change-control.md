...
| Security Debt Register(Task 5) | **C0**(security triage 文档;无代码/依赖变化) | in progress;`docs/security/security-debt-register.md` 基于真实 dependabot API(15 open:4H/8M/3L,14 dev-scope/1 runtime);每个 high 有 owner/risk/next-action;**仅 triage,本 slice 不做任何升级**;独立 track,不混 D8/observability/graph |
