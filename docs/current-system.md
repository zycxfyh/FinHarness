...
- `task acceptance:agent-shell-process-recovery` kills the API process after Runtime completion but before the product receipt is completed, restarts over the same stores, typed-reconciles the pending mutation, and proves one EffectIntent, one EffectExecution, and the same Runtime Job. It uses the real systemd Runtime on a capable root host and a persisted deterministic Runtime port on hosted runners without systemd; both modes execute the same simulated Execution Kernel and persist the observation required after restart.
- Each Mission exposes `/world-drift`; a changed admitted World blocks Effects with `mission_world_changed` until the user performs the keyed `checkpoint_and_resume` recovery. The recovery creates one deterministic checkpoint, advances the Mission World basis while keeping the Mission active, and has its own typed resolver.
- Typed identity-mutation contracts are aggregated by `api/identity_mutation_reconciliation.py`; Proposal and Agent route modules only own their domain contracts and handlers.
- `task acceptance:agent-shell-provider-redlines` evaluates the shared provider-output redline against a pinned English, Chinese, and Japanese corpus. The current corpus has 12 cases: 9 prohibited action/advice outputs and 3 allowed descriptive outputs.
- The product browser workflow runs a real Chromium Agent Shell journey through FastAPI, authenticated identity, Mission, structured paper Effect, ExecutionReport, PositionDelta, Reconciliation, and replay. Hosted browser CI uses a persisted deterministic Runtime port because GitHub runners do not provide the root-owned systemd/cgroup environment; `task acceptance:agent-shell` separately proves the real Rust Runtime and systemd Worker path.
