...
Date: 2026-06-13
Status: superseded for product direction by docs/product-north-star.md (2026-06-17)
...
This ADR remains useful as a historical correction against adding more
governance graphs, but it is no longer the product roadmap. The governing
product source of truth is [docs/product-north-star.md](../product-north-star.md).

The correction is:

```text
Product B0 = personal financial situational awareness and decision support.
B1-B5 = safety/quality predicates that support B0.
B4 = long-term learning / lesson-to-rule mechanism, not the project's existence
     reason and not the user-facing product value.
```

Do not cite this ADR to justify building a governance machine. Governance
objects must serve the cockpit: state visibility, risk awareness, candidate
options, blocked actions, and post-decision memory.
...
2026-06-12  docs/think/2026-06-12-target-state-b-and-loop-topology.md
  ...
  Section 8 rules out new governance graphs ("governance is the most mature
  part already"). It then defined a trading-slice B-root and predicates B1-B5;
  this ADR originally treated B4 as the discriminating product predicate. That
  part is superseded by the 2026-06-17 product north star.
```
...
The 2026-06-12 target-state-B document was accepted as the governing roadmap on
2026-06-13. As of 2026-06-17, it is superseded for product direction by
`docs/product-north-star.md`; the 2026-06-04 maturity roadmap remains a
**keep-warm backlog**.

```text
- Current north star: B0 — personal financial situational awareness and
  decision support. B1/B2/B3/B4 are safety/quality predicates; B5 is a permanent
  boundary.
...
H2 DONE  B4 loop closed AND enforced. Then treated as the north-star learning
         predicate; now a supporting predicate under product B0.
...
### Option 2: B-doc governs; maturity roadmap is keep-warm backlog (chosen on 2026-06-13)

```text
- one direction, then anchored on B4 as a learning predicate
...
```text
+ Historically: work sequenced toward B4 instead of toward more governance.
+ Current product frame: B4 supports B0 cockpit value; it is not the product end.
...
Working if: future artifacts serve product B0 cockpit value while keeping B4
lineage as a support predicate, not as a standalone governance end; no new
governance graph is added without an ADR overriding this one.

## Links

```text
docs/think/2026-06-12-target-state-b-and-loop-topology.md  (historical)
docs/product-north-star.md                                 (current product source of truth)
