...
北极星指标 = **财务决策清晰度**,而不是收益率/跑赢指数/推荐准确率/交易次数
(这些会把产品带歪成交易终端)。它把上面那句北极星拆成七个可观察维度:
...
不是「个人 cockpit vs 交易引擎」二选一,而是三层:

| 层 | 角色 | 回答 |
| --- | --- | --- |
| **前台 个人财务驾驶舱** | 产品的家 / 心智 | 我有什么、变了什么、风险在哪、现金够不够、今天看什么 |
| **中台 资本分配工作流** | 决策候选(阶段 3+) | 存量怎么调?增量怎么投?现金/债务/保险/税务/投资如何权衡? |
| **后台 研究/交易引擎** | **headless evidence provider** | 只为某个候选提供**带证据等级**的证据,不主导前端、不暴露买卖按钮、无执行权 |

**依赖方向(硬约束):** 候选 →(拉)→ 证据 ←(由)← 研究引擎。研究引擎**不**
驱动界面;其输出只能以「挂在某个候选下、带证据等级、历史描述而非预测」的
evidence 形态出现。不能表达成此形态的复杂度,留在 headless,不上前端。

**模式 = 家 + 工具抽屉,不是双主页。** 资产管理是产品心智的家;交易/研究是从
某个候选的 evidence detail 钻进去取证据的工具面,不得与个人 cockpit 平起平坐
成为对等主页(否则产品心智滑向交易终端)。
