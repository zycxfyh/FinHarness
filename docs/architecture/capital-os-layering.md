...
3. **L6**:`ActionIntentCandidate` 已提供 proposal → future capital action 的
   ...
   AgentAuthorityGrant 之后的 SuitabilityCheck / AuthorityContract 设计,再决定
   是否需要 OrderTicketCandidate。
4. **Authority path**:任何未来 AgentAuthorityGrant、SuitabilityCheck、
   ...
   显式说明豁免;CapitalMandate 是政策域,AgentAuthorityGrant 是受限 authority
   credential,两者都不是 execution authorization。
