...
| **L6** | Pre-/Post-trade 行动模拟与复盘 | 做这个动作会怎样?做完如何? | `statecore/action_intents.py`、`statecore/action_intent_authority_bindings.py`、`statecore/action_intent_simulations.py`、`statecore/trade_plan_candidates.py`、`statecore/trade_plan_review_gates.py`、`action_intent_preflight.py`、`api/routes_action_intents.py` | 🟡 `ActionIntentCandidate` + `ActionIntentAuthorityBinding` + system preflight + `ActionIntentSimulationReport` + `TradePlanCandidate` + `TradePlanReviewGate` v0 已有;order ticket / broker command path 仍是 gap |
...
3. **L6**:`ActionIntentCandidate` 已提供 proposal → future capital action 的
   candidate-only bridge;`ActionIntentAuthorityBinding` 已把
   AgentAuthorityGrant 接入 action-intent admission,记录 agent/human/system author
   的 allow/deny、requested scope、validated scope、binding/grant deny reason 和
   receipt lineage;system preflight 已消费 latest/current authority binding,
   对 agent-authored intent 缺 allowed binding、binding denied、binding receipt
   stale 或缺 grant validation evidence 的情况 block,同时继续重算 freshness、
   policy、scope、evidence、preconditions 和 risk posture;preflight-bound
   `ActionIntentSimulationReport`、`TradePlanCandidate` 与 `TradePlanReviewGate`
   已能把动作意图推进到描述性模拟、候选 pre-trade plan 和 human review-gated
   order-ticket-candidate staging eligibility。下一步不是直接 broker adapter,而是
   OrderTicketCandidate / BrokerSubmissionGate。
