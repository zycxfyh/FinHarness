...
| **L5** | Agent / 个人资本 Agent | 这些状态和提案是什么意思? | `agent_context.py`、`agent_capabilities.py`、`agent_tools.py`、proposal review surface | ✅ v0:context packs + default read/explain profile + review-draft proposal drafts + review provenance |
...
2. **L5**:把 context packs 用在更好的 Agent 解释/eval 中;review-draft profile
   ...
   Agent provenance,但不是 approval、recommendation 或 execution authorization。
