...
3. **L6**:`ActionIntentCandidate` 已提供 proposal → future capital action 的
   ...
   receipt lineage;system preflight 会重算 freshness、policy、scope、evidence、
   preconditions 和 risk posture;preflight-bound `ActionIntentSimulationReport`
   与 `TradePlanCandidate` 已能把动作意图推进到描述性模拟和候选 pre-trade
   plan。下一步不是直接 broker adapter,而是 authority-aware preflight,再进入
   TradePlanReviewGate / OrderTicketCandidate / BrokerSubmissionGate。
