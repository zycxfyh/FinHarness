...
状态:current(2026-07-02)。本页只描述**当前 mainline** 的模块事实。历史
...
| Capital Action Intent | current | `statecore/action_intents.py`, `statecore/action_intent_authority_bindings.py`, `statecore/action_intent_simulations.py`, `statecore/trade_plan_candidates.py`, `statecore/capital_objective_fits.py`, `statecore/trade_plan_review_gates.py`, `action_intent_preflight.py`, `api/routes_action_intents.py`, `ActionIntent`, `ActionIntentAuthorityBinding`, `ActionIntentSimulationReport`, `TradePlanCandidate`, `CapitalObjectiveFit`, `TradePlanReviewGate` | Receipt-backed `ActionIntentCandidate` bridge from current proposal receipt to future action preflight/simulation; `ActionIntentAuthorityBinding` records agent/human/system authority admission with structured binding/grant deny reasons and links ActionIntent → AgentAuthorityGrant → CapitalMandate where present; preflight consumes latest/current authority binding state, blocks agent-authored intents without allowed/current binding evidence, and recomputes freshness, policy, scope, evidence, impact summary, risk posture, and deterministic report hash; simulation reports bind current action intent receipt plus current preflight hash into qualitative downstream evidence; trade plan candidates bind simulation receipt plus current preflight evidence into candidate-only pre-trade plan scope; capital objective fits bind the same current evidence to objective alignment, benefit thesis, risk/liquidity/concentration impact, reversibility, opportunity cost, alternatives, uncertainties, and next safe path; trade plan review gates persist human allow/deny evidence for future order-ticket-candidate staging. Not investment advice, order ticket, broker submission, suitability certification, authority contract, or execution authorization. |
| Paper Validation Runtime | current | `statecore/paper_order_tickets.py`, `statecore/paper_executions.py`, `statecore/paper_accounts.py`, `api/routes_paper_validation.py`, `PaperOrderTicketCandidate`, `PaperExecutionReceipt`, `PaperAccount`, `PaperPosition` | Paper-only runtime v0: reviewed trade plans can become paper order ticket candidates, simulated fills/rejections, and isolated paper account/position state. Rejected executions carry no fill/notional/fees and require a rejection reason; account application rejects stale receipts, replay, insufficient cash/position, and live/broker markers. |
...
| L5 Paper Validation Runtime | current + planned | `PaperPerformanceReview`, `PaperValidationSession`, `ScenarioVsPaperComparison`, `PaperPnLSeries`, `PaperLessonCandidate` | Runtime v0 exists; performance/review loop is planned. |
| L6 Agent Task Runtime | planned | `AgentTask`, `AgentPlan`, `AgentStep`, `AgentToolRun`, `AgentArtifact`, `AgentCheckpoint`, `HumanHandoff` | Agent explanation/runtime v0 exists; task runtime artifacts are planned. |
