...
| Agent Cognition Runtime / Work Orchestrator | `current` | `src/finharness/agent_shell.py`<br>`src/finharness/api/routes_agent_shell.py`<br>`src/finharness/api/identity_mutation_reconciliation.py`<br>`src/finharness/agent_shell_provider_redline_eval.py`<br>`scripts/agent_shell_fixture_runtime.py`<br>`scripts/serve_local_agent.py`<br>`frontend-agent/`<br>`src/finharness/capital_agent.py`<br>`src/finharness/capital_runtime.py`<br>`src/finharness/runtime_worker.py`<br>`crates/finharness-runtime/`<br>`src/finharness/agent_cognition_flow.py`<br>`src/finharness/agent_run_receipts.py`<br>`src/finharness/agent_runtime_receipts.py`<br>`src/finharness/agent_tool_registry.py`<br>`src/finharness/agent_tool_availability.py`<br>`src/finharness/agent_tool_result_envelope.py`<br>`src/finharness/agent_context_trust_map.py`<br>`src/finharness/agent_receipt_search.py`<br>`src/finharness/domain_memory.py`<br>`src/finharness/playbook_loader.py`<br>`src/finharness/evaluator_registry.py`<br>`src/finharness/agent_operating_flow.py`<br>`src/finharness/review_workspace.py`<br>`src/finharness/agent_work_loop.py` | Python owns capital and Agent semantics; the local Agent Shell only orchestrates authenticated product journeys. A domain-neutral typed-reconciliation registry dispatches to Proposal and Agent handlers without making either route module the global owner. The transplanted Rust kernel owns execution identity, idempotency, process lifecycle, bounded Artifacts, and recovery. Paper Effects survive both acknowledgement loss and API-process death after Runtime completion without redispatch. Mission World drift requires an explicit checkpoint/resume before another Effect. Registered operations prevent model-selected executables or environment, browser sessions never receive provider secrets, and broker/capital reconciliation remain domain-owned. |
