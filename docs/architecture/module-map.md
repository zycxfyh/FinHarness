...
| Capital Action Intent | current | `statecore/action_intents.py`, `api/routes_action_intents.py`, `ActionIntent` | Receipt-backed `ActionIntentCandidate` bridge from current proposal receipt to future action preflight/simulation; not an order, broker action, approval, or execution authorization. |
