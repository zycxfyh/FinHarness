...
| IPS / Policy | current | `ips.py`, `api/routes_ips.py`, `statecore/capital_mandates.py`, `api/routes_capital_mandates.py`, `InvestmentPolicyStatement`, `CapitalMandate` | L3 v0. Active IPS personalizes L4 `ObservationThresholds`; compliance check is descriptive. CapitalMandate records the human-attested policy domain future authority objects may cite; it is not execution authorization or an Agent identity grant. |
