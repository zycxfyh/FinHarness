...
| Agent Explanation | current v0 | `agent_context.py`, `agent_capabilities.py`, `agent_evidence.py`, `agent_tools.py`, `agent_runtime.py`, `proposal_queue_checks.py`, `hermes_bridge.py` | Capital OS context packs plus explicit capability profiles, `AgentToolEntry` metadata/availability, evidence provider registry, runtime resolver/dispatch result/error/evidence envelope/budget, proposal draft provenance, read-only queue checks with blocked transition scope, and review-task lifecycle projections; default remains read/explain, review-draft can create append-only proposal drafts, and no execution authority exists. |
