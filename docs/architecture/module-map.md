...
| Capital Action Intent | current | `statecore/action_intents.py`, `action_intent_preflight.py`, `api/routes_action_intents.py`, `ActionIntent` | Receipt-backed `ActionIntentCandidate` bridge from current proposal receipt to future action preflight/simulation; read-only preflight reports recompute freshness, policy, scope, evidence, impact summary, risk posture, and deterministic report hash. Not an order, broker action, approval, or execution authorization. |
