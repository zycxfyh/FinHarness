...
| IPS / Policy / Authority Credentials | current | `ips.py`, `api/routes_ips.py`, `statecore/capital_mandates.py`, `api/routes_capital_mandates.py`, `statecore/agent_authority_grants.py`, `api/routes_agent_authority_grants.py`, `InvestmentPolicyStatement`, `CapitalMandateVersion`, `CapitalMandateLifecycleEvent`, `AgentAuthorityGrant` | L3 v0. Active IPS personalizes L4 `ObservationThresholds`; compliance check is descriptive. CapitalMandate is immutable, principal/time-resolved, typed-limit policy with append-only suspend/resume/revoke history; its old row is a compatibility projection. AgentAuthorityGrant remains a receipt-backed mandate-bound credential pending AUTH-03 migration. Neither object authorizes execution, approves trade plans, submits orders, or bypasses preflight. |
...
| Agent Cognition Runtime / Agent Operating Cycle v0.1 | current AUT2 foundation | `agent_cognition_flow.py`, `agent_run_receipts.py`, `agent_runtime_receipts.py`, `agent_tool_registry.py`, `agent_tool_availability.py`, `agent_tool_result_envelope.py`, `agent_context_trust_map.py`, `agent_receipt_search.py`, `domain_memory.py`, `playbook_loader.py`, `evaluator_registry.py`, `agent_operating_flow.py`, `review_workspace.py`, `agent_work_loop.py` | The 15-contract gate proves typed arguments, observation-driven decisions, independent budgets, playbook preflight, autonomy admission, terminal AgentRunReceipt/WorkResult/search/workspace, and all declared stop reducers. Session, resume, scheduling, subagents, and AUT3 authority remain later programs. |
| Agent Autonomy Control Plane | current AUT2 foundation | `autonomy_control.py`, `agent_autonomy_adapter.py`, `agent_work_loop.py` | Typed W0-W4/AUT0-AUT6 action admission plus a compatibility adapter for existing CapitalMandate and AgentAuthorityGrant records. Actual work-loop dispatch fails closed through persisted admission evidence and terminal linkage, while the Harness-owned runtime ceiling stays separate from the Agent request. Reports cannot execute or transition authority; effect commands are not integrated. |
