...
| Agent Explanation | current v0 | `agent_context.py`, `agent_capabilities.py`, `agent_tools.py`, `hermes_bridge.py` | Capital OS context packs plus explicit capability profiles and runtime tool registry/factory; default remains read/explain, review-draft can create append-only proposal drafts, and no execution authority exists. |
