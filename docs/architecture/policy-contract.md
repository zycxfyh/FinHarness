...
| AUTH-BROWSER-001 | `src/finharness/identity.py`, `src/finharness/api/app.py`, `frontend/api.js` | A durable Cockpit mutation attempt may be matched, retried, or cleared only under the same server-derived authenticated principal and authentication epoch. | Unknown, expired, legacy, corrupt, ambiguous, mismatched, or unacknowledged bindings retain evidence and fail before a new browser mutation fetch or cleanup. | Authentication proves browser retry identity only; domain Authority and execution capability remain separate. |
