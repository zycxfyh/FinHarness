...
FinHarness 的 Agent 工具应该逐步走向 `AgentToolEntry` 风格:
...
- Use this reference when strengthening profile-aware tool description and review
  exposure.
...
- Add dynamic availability checks when tools depend on optional environment,
  provider credentials, local state paths, or policy toggles.
