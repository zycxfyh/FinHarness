...
|| Agent Cognition Runtime v0 | Wave 0: Agent Tool Runtime → Agent Cognition Runtime。7 个新原语 (receipt-only / projection-only):AgentRunReceipt (Trace)、ContextTrust (Context)、EvaluationReport (Evaluation)、AuthorityTransitionRecord (Authority)、PlanningPolicyView (Feedback)、OptionSetReceipt / PlanDraftReceipt (Deliberation)。8 维 agentic target space 定义在 `agent-native-target-space.md`。Execution Kernel 冻结。 | `architecture/agent-native-target-space.md` | `agent_run_receipts.py`, `context_trust.py`, `evaluation_report.py`, `authority_transition.py`, `planning_policy_view.py`, `deliberation_receipts.py` | receipt-first / projection-first graduation rule;所有新原语不带 StateCore 表;不接 Execution Kernel | agent_run_receipts / context_trust / evaluation_report / authority_transition / planning_policy_view / deliberation_receipts tests |
