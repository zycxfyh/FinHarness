...
| Capital Action Intent | receipt-backed `ActionIntentCandidate` 把当前 proposal receipt 翻译成 future capital action intent;`ActionIntentAuthorityBinding` 记录 agent/human/system author admission 并保留 binding/grant deny reasons;system preflight/impact report 会消费 latest/current binding evidence 并重算 freshness、policy、scope、evidence 和 risk posture;preflight-bound simulation report 会绑定 current action intent receipt 和 current preflight hash;TradePlanCandidate 只描述 pre-trade plan;TradePlanReviewGate 记录 human allow/deny for future order-ticket-candidate staging,不生成 order ticket、不提交 broker、不授权 execution。 | `system-map.md`, `reference/interfaces.md`, `docs/reference/financial-terminology-map.md`, `docs/adr/2026-07-03-action-intent-authority-bindings-admit-only-to-next-governance-step.md`, `docs/adr/2026-07-03-financial-terminology-alignment.md` | `statecore/action_intents.py`, `statecore/action_intent_authority_bindings.py`, `statecore/action_intent_simulations.py`, `statecore/trade_plan_candidates.py`, `statecore/trade_plan_review_gates.py`, `action_intent_preflight.py`, `api/routes_action_intents.py` | OMS/EMS 前置 action-intent / command-intent + authority admission + pre-trade controls + supervisory review gate 分层;每层只授予进入下一治理步骤的资格,不授予越级执行能力;金融/监管类比不是 broker/adviser/compliance claim | action-intent + authority-binding tests |
