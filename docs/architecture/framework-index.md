...
| State Core | `current` | Receipt-backed query mirror for personal capital state; receipts remain the evidence root. | `src/finharness/statecore/`<br>`src/finharness/api/routes_state.py` | `task test:all`<br>`task governance:check` |
