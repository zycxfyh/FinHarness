...
| Agent Explanation | Agent 通过 context packs、capability profiles、`AgentToolEntry` 和 runtime pipeline 选择 actual tools;runtime 暴露 visible/hidden/unavailable tools、structured result/error、result budget;default profile 是 read/explain,review-draft 可创建 append-only governed proposal draft 并暴露 queue checks 和 review-task lifecycle projection,不审批、不执行。 | `system-map.md`, `reference/interfaces.md` | `agent_context.py`, `agent_capabilities.py`, `agent_tools.py`, `agent_runtime.py`, `proposal_queue_checks.py`, `hermes_bridge.py` | Hermes-style spec/availability/dispatch/budget/profile/toolset 思路;draft proposal 是 review object,queue check / review task 都不是 approval,capability profile/tool availability 不是 permission bypass | agent proposal/capability/context/runtime/tool tests, future evals |
