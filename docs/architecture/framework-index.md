...
| Agent Explanation | Agent 通过 context packs、capability profiles、`AgentToolEntry`、evidence provider registry、context projection policy 和 runtime pipeline 选择 actual tools;runtime 暴露 visible/hidden/unavailable tools、structured result/error/evidence envelope、profile-aware context budget、result budget;default profile 是 read/explain baseline,review-draft 可创建 append-only governed proposal draft 并暴露 queue checks 和 review-task lifecycle projection,不靠 prompt 承诺升级权限。 | `system-map.md`, `reference/interfaces.md` | `agent_context.py`, `agent_context_projection.py`, `agent_capabilities.py`, `agent_evidence.py`, `agent_tools.py`, `agent_runtime.py`, `proposal_queue_checks.py`, `hermes_bridge.py` | Hermes-style spec/availability/dispatch/context budget/profile/toolset/provider registry 思路;context projection 是能力释放控制面,draft proposal 是 review object,queue check / review task / evidence envelope 都不是 approval | agent proposal/capability/context/projection/evidence/runtime/tool tests, future evals |
