...
|| Agent Cognition Runtime v0 | Wave 0: Agent Tool Runtime → Agent Cognition Runtime。7 个新原语:AgentRunReceipt (Trace)、ContextTrust (Context)、EvaluationReport (Evaluation)、AuthorityTransitionRecord (Authority)、PlanningPolicyView (Feedback)、OptionSetReceipt / PlanDraftReceipt (Deliberation)。Wave 1: first deterministic cognition flow via `agent_cognition_flow.py` — goal→option set→plan draft→evaluation→authority→agent run receipt。Wave 1.1: projection/trust cleanup。Wave 1.2: semantic hardening — real plan evaluator, evaluation→authority eligibility policy, context trust use validation。`scripts/run_agent_cognition_flow_smoke.py` 可本地验证全链路。8 维 agentic target space 定义在 `agent-native-target-space.md`。Execution Kernel 冻结。 | `architecture/agent-native-target-space.md` | `agent_cognition_flow.py`, `agent_run_receipts.py`, `context_trust.py`, `evaluation_report.py`, `authority_transition.py`, `planning_policy_view.py`, `deliberation_receipts.py` | receipt-first / projection-first graduation rule;所有新原语不带 StateCore 表;不接 Execution Kernel | agent_cognition_flow / agent_run_receipts / context_trust / evaluation_report / authority_transition / planning_policy_view / deliberation_receipts tests |
