...
| Agent Explanation | Agent 通过只读 context packs 和 capability profiles 选择可见工具;default profile 是 read/explain,不裸读全库、不写核心状态。 | `system-map.md`, `reference/interfaces.md` | `agent_context.py`, `agent_capabilities.py`, `agent_tools.py`, `hermes_bridge.py` | OpenAI Agents/tool calling 作为解释层;Hermes-style spec/availability/budget/profile 思路,capability profiles 不是 permission bypass | agent capability/context/tool tests, future evals |
