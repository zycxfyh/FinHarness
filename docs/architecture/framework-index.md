...
||| Agent Cognition Runtime v0.93 | Wave 0–2.2 delivered. 8D agentic space, deterministic cognition flow, semantic hardening, Agent Operating Surface v0.4 (12 PRs hardening), Agent Work Loop v0.1 (13 PRs). Smoke scripts: `run_agent_operating_surface_smoke.py` (24 checks), `run_agent_work_loop_smoke.py` (21 checks). Execution Kernel frozen. | `agent-work-loop-plan.md` | `agent_work_loop.py` orchestrates full cycle. | Receipt-first, trace-by-default, bounded budget, frozen snapshot. No session/scheduler/execution. | all agent + work loop tests |
