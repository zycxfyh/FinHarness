...
| Capital Action Intent | receipt-backed `ActionIntentCandidate` 把当前 proposal receipt 翻译成 future capital action intent,但不生成 order、broker action、simulation 或 execution authority。 | `system-map.md`, `reference/interfaces.md` | `statecore/action_intents.py`, `api/routes_action_intents.py` | OMS/EMS 前置 action-intent / command-intent 分层;先定义动作意图,再做 preflight/simulation/order ticket | action-intent tests |
