...
| Agent Explanation | Agent 通过 context packs、capability profiles、`AgentToolEntry`、evidence provider registry 和 runtime pipeline 选择 actual tools;runtime 暴露 visible/hidden/unavailable tools、structured result/error/evidence envelope、result budget;default profile 是 read/explain,review-draft 可创建 append-only governed proposal draft 并暴露 queue checks 和 review-task lifecycle projection,不审批、不执行。 | `system-map.md`, `reference/interfaces.md` | `agent_context.py`, `agent_capabilities.py`, `agent_evidence.py`, `agent_tools.py`, `agent_runtime.py`, `proposal_queue_checks.py`, `hermes_bridge.py` | Hermes-style spec/availability/dispatch/budget/profile/toolset/provider registry 思路;draft proposal 是 review object,queue check / review task / evidence envelope 都不是 approval,capability profile/tool availability 不是 permission bypass | agent proposal/capability/context/evidence/runtime/tool tests, future evals |
