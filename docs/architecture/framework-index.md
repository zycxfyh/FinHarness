...
| Agent Explanation | Agent 通过 context packs、capability profiles 和 runtime registry/factory 选择 actual tools;default profile 是 read/explain,review-draft 可创建 append-only governed proposal draft 并暴露带 blocked transition scope 的 queue checks,不审批、不执行。 | `system-map.md`, `reference/interfaces.md` | `agent_context.py`, `agent_capabilities.py`, `agent_tools.py`, `proposal_queue_checks.py`, `hermes_bridge.py` | Hermes-style spec/availability/budget/profile/toolset 思路;draft proposal 是 review object,queue check 不是 approval,capability profile 不是 permission bypass | agent proposal/capability/context/tool tests, future evals |
