...
状态:current(2026-06-28)。这是 FinHarness 的“不要每次重新理解一遍”索引。
...
| Review System | append-only proposal review、scaffold revision、attestation、compare、annual review、lesson-to-rule。 | `system-map.md`, `engineering/system-directory-standard.md` | `statecore/proposals.py`, `review_read.py`, `lesson_loop.py` | GitLab review discipline + decision log;不把 attestation 当 execution | `GOV-REVIEW-001`, review tests |
...
| Agent Explanation | Agent 通过 context packs、capability profiles 和 `AgentToolEntry` registry/factory 选择 actual tools;tool metadata 暴露 capability、toolset、side-effect、availability;default profile 是 read/explain,review-draft 可创建 append-only governed proposal draft 并暴露 queue checks 和 review-task lifecycle projection,不审批、不执行。 | `system-map.md`, `reference/interfaces.md` | `agent_context.py`, `agent_capabilities.py`, `agent_tools.py`, `proposal_queue_checks.py`, `hermes_bridge.py` | Hermes-style spec/availability/budget/profile/toolset 思路;draft proposal 是 review object,queue check / review task 都不是 approval,capability profile/tool availability 不是 permission bypass | agent proposal/capability/context/tool tests, future evals |
