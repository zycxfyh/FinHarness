...
| State Core | `current` | Receipt-backed query mirror for personal capital state with exact Decimal ingestion, explicit UTC clocks, completeness findings, and immutable evidence roots. | `src/finharness/statecore/`<br>`src/finharness/capital_import_contract.py`<br>`src/finharness/personal_finance.py`<br>`src/finharness/beancount_adapter.py`<br>`src/finharness/api/routes_state.py` | `task test:all`<br>`task governance:check` |
