...
| Agent Explanation | Agent 通过只读 context packs 和 tool 解释状态/IPS/proposal/review/风险,不裸读全库、不写核心状态。 | `system-map.md`, `reference/interfaces.md` | `agent_context.py`, `agent_tools.py`, `hermes_bridge.py` | OpenAI Agents/tool calling 作为解释层;Hermes-style spec/availability/budget 思路,不做 source-of-truth | agent context/tool tests, future evals |
