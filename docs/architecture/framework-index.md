...
|| Agent Cognition Runtime v0 | Wave 0: Agent Tool Runtime → Agent Cognition Runtime。7 个新原语:AgentRunReceipt (Trace)、ContextTrust (Context)、EvaluationReport (Evaluation)、AuthorityTransitionRecord (Authority)、PlanningPolicyView (Feedback)、OptionSetReceipt / PlanDraftReceipt (Deliberation)。Wave 1: first deterministic cognition flow via `agent_cognition_flow.py` — goal→option set→plan draft→evaluation→authority→agent run receipt。Wave 1.1: projection/trust cleanup。Wave 1.2: semantic hardening — real plan draft semantic evaluator, authority eligibility policy, context use policy primitive + ContextTrust profiles。Wave 1.3: semantic integration closure — evaluation override gated (evaluator_override + allow_evaluation_override), tokenization upgraded to regex boundary + phrase allowlist, ContextUsePolicy consumed by AgentCognitionFlow, context trust findings affect EvaluationReport.status and AuthorityTransition.eligibility。`scripts/run_agent_cognition_flow_smoke.py` 可本地验证全链路。8 维 agentic target space 定义在 `agent-native-target-space.md`。Wave 2 RFC 定义在 `agent-operating-surface.md`。Execution Kernel 冻结。 | `architecture/agent-native-target-space.md`, `architecture/agent-operating-surface.md` | `agent_cognition_flow.py`, `agent_run_receipts.py`, `context_trust.py`, `context_use_policy.py`, `evaluation_report.py`, `authority_transition.py`, `authority_eligibility_policy.py`, `plan_draft_evaluator.py`, `planning_policy_view.py`, `deliberation_receipts.py` | receipt-first / projection-first graduation rule;所有新原语不带 StateCore 表;不接 Execution Kernel | agent_cognition_flow / agent_run_receipts / context_trust / context_use_policy / evaluation_report / authority_transition / authority_eligibility_policy / plan_draft_evaluator / planning_policy_view / deliberation_receipts tests |
