...
| Capital Action Intent | receipt-backed `ActionIntentCandidate` 把当前 proposal receipt 翻译成 future capital action intent;system preflight/impact report 会重算 freshness、policy、scope、evidence 和 risk posture,但不生成 order、broker action、simulation 或 execution authority。 | `system-map.md`, `reference/interfaces.md` | `statecore/action_intents.py`, `action_intent_preflight.py`, `api/routes_action_intents.py` | OMS/EMS 前置 action-intent / command-intent + pre-trade admission controller 分层;先定义动作意图,再做 preflight/simulation/order ticket | action-intent tests |
