...
|| Agent Cognition Runtime v0 | Wave 0: Agent Tool Runtime → Agent Cognition Runtime。7 个新原语。Wave 1: deterministic cognition flow, semantic hardening, integration closure。Wave 2: Agent Operating Surface — AgentToolRegistry v0, availability snapshots, tool result envelopes, runtime receipt bridge, context trust map extraction, receipt search, domain memory, CognitionPlaybooks, evaluator registry, research evidence evaluator, operating flow bridge, review workspace projection。`scripts/run_agent_operating_surface_smoke.py` 验证全链路。8 维 agentic target space 在 `agent-native-target-space.md`，operating surface 定义在 `agent-operating-surface.md`。Execution Kernel 冻结。 | `architecture/agent-native-target-space.md`, `architecture/agent-operating-surface.md` | `agent_cognition_flow.py`, `agent_run_receipts.py`, `agent_runtime_receipts.py`, `agent_tool_registry.py`, `agent_tool_availability.py`, `agent_tool_result_envelope.py`, `agent_context_trust_map.py`, `agent_receipt_search.py`, `domain_memory.py`, `playbook_loader.py`, `evaluator_registry.py`, `research_evidence_evaluator.py`, `agent_operating_flow.py`, `review_workspace.py`, `context_trust.py`, `context_use_policy.py`, `evaluation_report.py`, `authority_transition.py`, `authority_eligibility_policy.py`, `plan_draft_evaluator.py`, `planning_policy_view.py`, `deliberation_receipts.py` | receipt-first / projection-first graduation rule;所有新原语不带 StateCore 表;不接 Execution Kernel | all agent tests |
