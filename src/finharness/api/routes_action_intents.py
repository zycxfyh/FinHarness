...
class PaperExecutionCreateRequest(BaseModel):
    ...
    @field_validator("expected_paper_order_ticket_receipt_ref", "fill_price", "simulator_ref")
    @classmethod
    def require_non_blank_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("paper execution requires ticket receipt, fill price, and simulator")
        return value
