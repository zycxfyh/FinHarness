...
def build_reconciliations(
    ...
    for request in execution_snapshot.order_requests:
        ...
        filled_quantity = max((item.filled_quantity for item in events), default=0)
        requested_quantity = requests_by_id[request.order_request_id].quantity
...
def build_cost_estimates(
    *,
    ...
    for reconciliation in reconciliations:
        ...
        reference_price = request.reference_price if request else None
        ...
        if reference_price is None:
            notes.append("reference price missing")
        if fill_price is None:
            notes.append("average fill price missing")
        slippage_per_unit = (
            fill_price - reference_price
            if reference_price is not None and fill_price is not None
            else None
        )
        slippage_total = (
        ...
        estimated_total_cost = (
            slippage_total + estimated_fees if slippage_total is not None else None
        )
        estimates.append(
