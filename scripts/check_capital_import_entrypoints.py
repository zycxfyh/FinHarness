...
def _reachable_call_names(tree: ast.Module, symbol: str) -> set[str]:
    return {call for node in _reachable_function_nodes(tree, symbol) for call in _call_names(node)}
...
def find_generic_write_bypasses(
    ...
    *,
    ...
    for function_name in sorted(_function_nodes(tree)):
        ...
        writers = sorted(
            {call for node in reachable_nodes for call in _call_names(node)}
    ...
    for path in _python_files(root=root):
        try:
            ...
            current = find_generic_write_bypasses(
                ...
                registered_materialized_sources=(PRODUCTION_CAPITAL_IMPORT_MATERIALIZED_SOURCES),
            )
        ...
        for finding in current:
            findings.append({**finding, "path": path.relative_to(root).as_posix()})
    return findings
...
def _exposure_findings(
    *, root: Path = ROOT, discovered: tuple[dict[str, str], ...] | None = None
    ...
    registered = {
        (spec.exposure_kind, spec.exposure_ref) for spec in PRODUCTION_CAPITAL_IMPORT_EXPOSURES
    }
...
def validate_capital_import_entrypoints(
    *,
    ...
    return {
        ...
        "production_materialized_sources": sorted(PRODUCTION_CAPITAL_IMPORT_MATERIALIZED_SOURCES),
