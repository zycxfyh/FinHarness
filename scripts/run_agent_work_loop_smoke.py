#!/usr/bin/env python3
"""Agent Work Loop semantic smoke v0.

Runs end-to-end: AgentWorkRequest -> context snapshot -> bounded dispatch ->
cognition flow -> evaluation -> workspace hydration -> search index update.
"""
...
def main() -> int:
    failures = 0
    print("Agent Work Loop Semantic Smoke")
    print("=" * 50)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        flow = run_cognition_flow_from_work_result(
            request=req, context_snapshot=snap,
            tool_envelopes=envelopes, receipt_root=root,
        )
        ...
        # 7. Full Work Loop
        print("\n7. Full Agent Work Loop")
        result = run_agent_work_loop(request=req)
        failures += _check("Result outcome in succeeded/partial",
                          result.outcome in ("succeeded", "partial"))
        ...
        propose_memory_from_completed_work(
            result=result, context_snapshot=snap, receipt_root=root,
        )
    ...
    print("\n" + "=" * 50)
    if failures == 0:
        print("ALL CHECKS PASSED — Agent Work Loop is operational.")
    else:
